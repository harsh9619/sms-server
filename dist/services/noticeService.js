...
export async function getNotices(schoolId, audience) {
    const result = await query(queries.GET_NOTICES, [schoolId, audience]);
    return result.rows;
}
export async function createNotice(schoolId, data) {
    const { title, content, audience, isPinned, createdBy } = data;
    const result = await query(queries.CREATE_NOTICE, [schoolId, title, content, audience || 'all', isPinned || false, createdBy || null]);
    return result.rows[0];
}
export async function updateNotice(noticeId, data) {
    const { title, content, audience, isPinned } = data;
    const result = await query(queries.UPDATE_NOTICE, [title, content, audience, isPinned, noticeId]);
