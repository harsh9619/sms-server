...
export async function updateSubjectTeacher(
  ...
  await query(
    "DELETE FROM school_subject_teachers WHERE school_id = $1 AND class_id = $2 AND subject_id = $3",
    [schoolId, classId, subjectMasterId]
  );
  ...
  if (teacherId) {
    await query(
      ...
      [schoolId, sayId || null, subjectMasterId, teacherId, classId]
    );
  }
}
...
export async function syncClassSubjects(
  schoolId: number,
  classId: number,
  masterSubjectIds: number[],
  sayId?: number | null
) {
  // 1. Fetch existing subject master IDs assigned to this class
  const existingRes = await query(
    "SELECT subject_id FROM school_class_subjects WHERE school_id = $1 AND class_id = $2",
    [schoolId, classId]
  );
    ...
    if (!existingMasterIds.has(masterId)) {
      await query(
        `INSERT INTO school_class_subjects (school_id, school_academic_year_id, class_id, subject_id)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT DO NOTHING`,
        [schoolId, sayId || null, classId, masterId]
      );
    }
  }
