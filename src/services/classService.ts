    ...
    if (subjects && Array.isArray(subjects)) {
      for (const sub of subjects) {
        ...
        const smRes = await query(
          `SELECT id FROM subject_masters WHERE LOWER(name) = LOWER($1) OR id::text = $1 LIMIT 1`,
          [cleanSub]
        );
        ...
        if (smId) {
          ...
          if (dbTeacherId) {
            await query(
              ...
              [schoolId, finalSayId, smId, dbTeacherId, newClass.id]
            );
          }
        }
      }
        ...
        if (smId) {
          ...
          if (dbTeacherId) {
            await query(
              `INSERT INTO school_subject_teachers (school_id, school_academic_year_id, subject_id, teacher_id, class_id)
               VALUES ($1, $2, $3, $4, $5) ON CONFLICT DO NOTHING`,
              [schoolId, finalSayId, smId, dbTeacherId, classId]
            );
          }
        }
      }
